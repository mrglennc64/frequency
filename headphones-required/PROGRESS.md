# Headphones Required — Progress Tracker

Statuses: `not started` → `drafted` → `reviewed` → `designed` → `done`
(Videos: `not started` → `scripted` → `produced` → `done`. Pages/assets: `not started` → `drafted` → `live`.)
...
| Full-book design pass + PDF export QA | — | W6 (Aug 10–16) | not started |
| Platform decision (Gumroad vs. own site) | `launch/funnel-checklist.md` | end of W3 | open |
| Funnel connected end-to-end, audit offer live | `launch/funnel-checklist.md` | Launch w/c Aug 17 | not started |
